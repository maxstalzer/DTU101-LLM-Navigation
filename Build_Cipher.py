...
def escape_string(s):
    """Escapes strings for safe Neo4j Cypher generation."""
    if not s: return ""
...
def build_neo4j_cypher_seed():
    ...
    cypher_statements = [
        ...
        "// Generated for Neo4j Import",
        "// ==========================================\n",
        
        "// 1. Setup Constraints to ensure data integrity",
    ...
    for node_id, attr in nodes_101.items():
        ...
        if floor_id not in floors_seen:
            ...
            # Fix: Isolate the MATCH/MERGE so variables don't bleed into the next statement
        ...
        if attr.get('is_wheelchair_accessible'): label += ":Accessible"
        
        name = escape_string(attr.get('name'))
        desc = escape_string(attr.get('description'))
        
        # Fix: Notice there is no variable 's' before the colon here. Just 'CREATE (:Space...'
        stmt = (
            f'CREATE (:{label} {{'
            f'poi_id: "{node_id}", name: "{name}", description: "{desc}", '
            f'maze_kind: "{attr.get("maze_kind")}", x: {x}, y: {y}, floor: {attr.get("z_level")}'
            f'}});'
        )
        cypher_statements.append(stmt)
        
        # Link Space to Floor
        cypher_statements.append(f'MATCH (s:Space {{poi_id: "{node_id}"}}), (f:Floor {{floor_id: "{floor_id}"}}) MERGE (f)-[:HAS_SPACE]->(s);')
    ...
    for floor_level, spaces in floors.items():
        for s1, s2 in itertools.combinations(spaces, 2):
            ...
            if p1.distance(p2) < 3.0:
                ...
                # Fix: Isolate variables by executing separate MATCH queries
                cypher_statements.append(
